...
/**
 * Script RESET database và tạo lại chỉ 500 users với phân bổ Pareto (80/20)
 *
 * CẢNH BÁO: Script này sẽ XÓA TẤT CẢ users và interactions cũ!
 *
 * CÁCH CHẠY:
 *   node backend/src/utils/resetAndSeedPareto.js
 */
...
async function createParetoUsers() {
...
async function main() {
  ...
  console.log("=".repeat(60));
  console.log("RESET DATABASE VÀ TẠO LẠI 500 USERS VỚI PARETO");
  console.log("=".repeat(60));
  ...
  try {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("Tạo 500 users mới với phân bổ Pareto...");
    console.log("=".repeat(60));

    await createParetoUsers();
    ...
    if (totalUsers === 500) {
      console.log(
        "✅ Thành công! Database chỉ còn 500 users với phân bổ Pareto"
      );
    } else {
      console.log(`⚠️  Cảnh báo: Có ${totalUsers} users thay vì 500`);
    }
  } catch (error) {
    ...
    console.log(
      "\n💡 Bước tiếp theo: Chạy exportInteractions.js để tạo interactions.csv mới"
    );
  }
}
